...
public class GameViewModel extends AndroidViewModel {
    ...
    public boolean checkAnswer(String userAnswer) {
        ...
        Log.d(TAG, "Checking Answer: User='" + userAnswer + "' (Normalized='" + normalizedUserAnswer + "') vs Correct='" + correctAnswer + "' (Normalized='" + normalizedCorrectAnswer + "')");

        if (current.getLevelId() == 1) {
            String normalizedLink = normalizeString("link");
            String normalizedTautan = normalizeString("tautan");
            String normalizedTautanLink = normalizeString("tautan (link)");

            isCorrect = normalizedUserAnswer.equals(normalizedLink) ||
                    normalizedUserAnswer.equals(normalizedTautan) ||
                    normalizedUserAnswer.equals(normalizedTautanLink);
            Log.d(TAG, "Chapter 1 Level 1 specific check: isCorrect=" + isCorrect);
        } else {
            // Logika umum untuk semua pertanyaan lain: Toleransi Typo dasar
            String[] correctWords = normalizedCorrectAnswer.split(" ");
            boolean containsAllCorrectWords = true;
            if (correctWords.length > 0) {
                for (String word : correctWords) {
                    if (!normalizedUserAnswer.contains(word)) {
                        containsAllCorrectWords = false;
                        break;
                    }
                }
            } else {
                containsAllCorrectWords = normalizedUserAnswer.isEmpty();
            }
            isCorrect = containsAllCorrectWords;
            Log.d(TAG, "General typo-tolerant check: isCorrect=" + isCorrect);
        }
    ...
    private String normalizeString(String text) {
        return text.trim().toLowerCase(Locale.getDefault()).replaceAll("[^a-z0-9]", "");
    }
    ...
    protected void onCleared() {
        ...
        if (_questionsForLevel != null && _questionsListObserver != null) {
            _questionsForLevel.removeObserver(_questionsListObserver);
            Log.d(TAG, "onCleared: Removed _questionsForLevel observer.");
        }
        if (_gameState != null) {
